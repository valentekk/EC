...
// vmwin
//
#include "../shared/vm_windows.h" // Inclui o cabeçalho para operações de VM no Windows
#include <TlHelp32.h> // Inclui funções para manipulação de processos
#pragma comment(lib, "ntdll.lib") // Linka a biblioteca ntdll.lib

// Declaração externa da função NtQueryInformationProcess
extern "C" __kernel_entry NTSTATUS NtQueryInformationProcess(
	HANDLE           ProcessHandle,             // Handle do processo a ser consultado
	ULONG            ProcessInformationClass,    // Classe da informação do processo que se deseja obter
	PVOID            ProcessInformation,         // Buffer para armazenar a informação retornada
	ULONG            ProcessInformationLength,   // Tamanho do buffer
	PULONG           ReturnLength                 // Tamanho da informação retornada
);
